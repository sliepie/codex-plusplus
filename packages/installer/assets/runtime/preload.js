...
function startSettingsInjector() {
  ...
  runInjectionProbe();
  for (const delayMs of [250, 1e3, 2500]) {
...
function runInjectionProbe() {
  tryInject();
  maybeDumpDom();
}
function scheduleInjectionProbe(mutations) {
...
function shouldInspectMutations(mutations) {
...
function nodeTouchesSettingsSurface(node, root) {
  return node === root || root.contains(node) || node.contains(root);
}
function isSettingsProbeSurface(node) {
...
function isDomProbeDebugEnabled() {
  try {
...
function clearSettingsSurfaceRefs() {
...
function shouldMirrorPreloadLog(level) {
  return level === "warn" || level === "error" || isPreloadDebugEnabled();
}
function sendPreloadLog(level, msg) {
  if (!shouldMirrorPreloadLog(level)) return;
  try {
    import_electron2.ipcRenderer.send("codexpp:preload-log", level, msg);
  } catch {
  }
}
...
async function mountManager(snapshot) {
  const tweaks = snapshot?.tweaks ?? await import_electron3.ipcRenderer.invoke("codexpp:list-tweaks");
  const paths = snapshot?.paths ?? await import_electron3.ipcRenderer.invoke("codexpp:user-paths");
  registerSection({
...
// src/preload/index.ts
function fileLog(stage, extra, level = "info") {
  if (level === "info" && !isPreloadDebugEnabled2()) return;
  ...
  try {
    import_electron4.ipcRenderer.send("codexpp:preload-log", level, msg);
  } catch {
  }
}
function isPreloadDebugEnabled2() {
  try {
    const debugWindow = window;
    return debugWindow.__codexppPreloadDebug === true || localStorage.getItem("codexpp:debug-preload") === "1";
  } catch {
...
async function boot() {
  ...
  try {
    ...
    fileLog("boot FAILED", String(e?.stack ?? e), "error");
  ...
  import_electron4.ipcRenderer.on("codexpp:tweaks-changed", () => {
    ...
    reloading = (async () => {
      try {
        ...
        fileLog("hot reload FAILED", String(e?.stack ?? e), "error");
